...
export function AdsTasks({
  ...
  return <>
    ...
    {!ads.data.activePackage ? <div className="panel grid min-h-52 place-items-center p-6 text-center text-sm font-semibold leading-6 text-amber-100">{t("noPackageAdsMessage")}</div> : <div className="grid grid-cols-2 gap-3">{ads.data.ads.map((ad: any) => { const locked = ad.state === "locked"; const watched = ad.state === "watched"; const active = session?.ad?.id === ad.id; return <article key={ad.id} className={`panel flex h-[140px] flex-col overflow-hidden p-3 ${locked ? "opacity-70" : ""}`}><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="eyebrow text-[9px]">{t("rewardedAd")} · #{ad.id}</p><h2 className="mt-0.5 truncate text-sm font-bold">{`${ad.title} - ${ad.timerSeconds} Sec - ${t("reward")} ${ad.rewardPkr} PKR`}</h2></div>{locked ? <LockKeyhole className="size-4 shrink-0 text-slate-400" /> : watched ? <CheckCircle2 className="size-4 shrink-0 text-emerald-300" /> : active ? <Clock3 className="size-4 shrink-0 text-amber-300" /> : <Play className="size-4 shrink-0 text-amber-300" />}</div><p className="mt-4 line-clamp-2 text-[11px] leading-4 text-slate-300">{`${ad.timerSeconds} Sec · ${t("reward")} ${ad.rewardPkr} PKR`}</p><div className="mt-auto pt-2">{locked ? <div className="flex h-8 items-center justify-center gap-1 rounded-lg border border-white/10 bg-slate-950/25 text-[10px] font-bold text-slate-400"><LockKeyhole className="size-3" />{t("lockedAd")}</div> : watched ? <div className="flex h-8 items-center justify-center gap-1 rounded-lg bg-emerald-400/15 text-[10px] font-bold text-emerald-200"><CheckCircle2 className="size-3" />{t("watched")}</div> : active ? <div className="rounded-lg border border-amber-300/30 bg-amber-300/10 px-2 py-1.5 text-center"><p className="text-sm font-bold text-amber-300">{seconds > 0 ? `Wait ${seconds}s…` : t("claimReward")}</p>{seconds === 0 && <button disabled={claim.isPending} onClick={() => claim.mutate({ sessionId: session.sessionId })} className="mt-1 h-7 rounded-md bg-amber-300 px-2 text-[10px] font-bold text-slate-950 disabled:opacity-50">{t("claimReward")}</button>}</div> : <button disabled={Boolean(session) || start.isPending} onClick={() => start.mutate({ slot: ad.id })} className="flex h-8 w-full items-center justify-center gap-1 rounded-lg bg-amber-300 text-[10px] font-bold text-slate-950 disabled:opacity-50"><Play className="size-3" />{t("watchAd")}</button>}</div></article>; })}</div>}
  </>;
}
