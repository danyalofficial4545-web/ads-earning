...
- The user explicitly declined continuous hosting. The shared game expansion must therefore use a server-validated, timestamp-seeded common round identifier and durable database records rather than `setInterval`, in-memory round state, or background timers.
- The new timestamp-shared Euro route retained the existing protected workspace loading shell in unauthenticated desktop and mobile previews; no blank route appeared. An authenticated member visual pass is still required to inspect live shared games, wager controls, and Ludo match interaction.
