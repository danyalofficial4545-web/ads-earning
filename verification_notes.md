...
Withdrawal and channel-reward update QA: the full regression suite passed with 75 tests passed and one opt-in Telegram connection test skipped. TypeScript, the standard production build, and the Vercel-specific build all passed. Two mobile preview captures reached the existing authenticated workspace loading screen without new console errors; therefore, the new member prompt and withdrawal-rule absence were additionally verified through focused source regression tests rather than the stale managed preview session.
