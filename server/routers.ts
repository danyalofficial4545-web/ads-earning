...
async function notifyUser(
  ...
  await db
    ...
    .values({ userId, title, message, isRead: false });
}
...
async function awardTaskReferralReward(
  db: any,
  referredId: number,
  taskId: number
) {
  ...
  const alreadyRewarded = (
    await db
      .select({ id: referralTaskRewards.id })
      .from(referralTaskRewards)
      .where(
        and(
          eq(referralTaskRewards.referredId, referredId),
          eq(referralTaskRewards.taskId, taskId)
        )
      )
      .limit(1)
  )[0];
  ...
  const referrer = (
    await db
      .select()
      .from(profiles)
      .where(eq(profiles.userId, relation.referrerId))
      .limit(1)
  )[0];
  if (!referrer) return;
  ...
  await db
    .update(profiles)
    .set({
      earningWalletBalance: (referrer.earningWalletBalance ?? 0) + rewardCoins,
    })
    .where(eq(profiles.userId, relation.referrerId));
  await db
    .update(referrals)
    .set({
      totalTasksRewarded: relation.totalTasksRewarded + 1,
      totalTaskEarnings: relation.totalTaskEarnings + rewardCoins,
    })
    .where(eq(referrals.id, relation.id));
  await db
    .insert(referralTaskRewards)
    .values({
  ...
  await db
    ...
    .values({
      ...
      amountPkr: 1,
      status: "completed",
      note: "Referral Task Reward: 1 PKR from referred user task",
      referenceType: "referral_task_reward",
...
export async function processTimewallPostback(input: {
  ...
  if (input.secret !== ENV.timewallPostbackSecret)
  ...
  if (
    ...
    userId <= 0 ||
    !Number.isInteger(coinsReceived) ||
    coinsReceived <= 0 ||
    !transactionId ||
  ...
  const duplicate = (
    await db
      .select({ id: timewallPostbacks.id })
      .from(timewallPostbacks)
      .where(eq(timewallPostbacks.transactionId, transactionId))
      .limit(1)
  )[0];
  if (duplicate) return { status: 200, body: "OK" };
  const coinsGivenToUser = Math.floor(coinsReceived * 0.1);
  ...
  await withDbTransaction(db, async tx => {
    ...
    await tx
      .insert(timewallPostbacks)
      .values({ userId, coinsReceived, coinsGivenToUser, transactionId });
    await tx
      .update(profiles)
      .set({
        earningWalletBalance:
          (profile.earningWalletBalance ?? 0) + coinsGivenToUser,
      })
      .where(eq(profiles.userId, userId));
    await tx
      .insert(transactions)
      .values({
        userId,
        type: "timewall_earning",
        direction: "credit",
        amountPkr: Math.floor(coinsGivenToUser / 100),
        status: "completed",
        note: `Timewall earning: ${coinsGivenToUser} coins (10% of ${coinsReceived})`,
        referenceType: "timewall_postback",
    ...
    if (relation && relation.totalTasksRewarded < relation.maxTaskReward) {
      ...
      if (referrer) {
        await tx
          .update(profiles)
          .set({
            earningWalletBalance: (referrer.earningWalletBalance ?? 0) + 100,
          })
          .where(eq(profiles.userId, relation.referrerId));
        await tx
          .update(referrals)
          .set({
            totalTasksRewarded: relation.totalTasksRewarded + 1,
            totalTaskEarnings: relation.totalTaskEarnings + 100,
          })
          .where(eq(referrals.id, relation.id));
        await tx
          .insert(referralTaskRewards)
        ...
        await tx
          ...
          .values({
            ...
            amountPkr: 1,
            status: "completed",
            note: "Referral Task Reward: 1 PKR from Timewall completion",
            referenceType: "timewall_postback",
            referenceId: transactionId,
          });
      }
...
function validateInlineUpload(
  ...
  options: {
    label: string;
    maxBytes: number;
    acceptedType: (contentType: string) => boolean;
  }
) {
  ...
  if (file.length === 0 || file.length > options.maxBytes)
    fail(
      `${options.label} must be between 1 byte and ${Math.floor(options.maxBytes / 1024 / 1024)} MB.`
    );
...
function saveAdMedia(raw: string, contentType: "image" | "video") {
...
async function buildOverview(userId: number) {
  ...
  const rewardProfile =
    rewardWithdrawalRequested && !profile.whatsappRewardWithdrawn
  ...
  const totalWithdrawals = await db
    ...
    .from(withdrawals)
    .where(
      and(eq(withdrawals.userId, userId), eq(withdrawals.status, "approved"))
    );
    ...
    register: publicProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!username) {
          ...
          if (!username)
            fail("Could not create a username. Please try again.", "CONFLICT");
        }
        ...
        if (deviceMatch[0] || networkMatch[0])
          fail(
            "You cannot create multiple accounts on same device, only one account allowed per device",
            "FORBIDDEN"
          );
    ...
    joinWhatsApp: protectedProcedure.mutation(async ({ ctx }) => {
      ...
      await db
        ...
        .set({
          ...
          earningWalletBalance:
            (profile.earningWalletBalance ?? 0) +
            WHATSAPP_JOIN_REWARD_PKR * 100,
        })
        .where(eq(profiles.userId, user.id));
        ...
        await db
          ...
          .set({
            depositWalletBalance:
              (profile.depositWalletBalance ?? 0) - purchasePriceCoins,
  ...
  task: router({
    ...
    get: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (!db)
          fail("Database is temporarily unavailable.", "INTERNAL_SERVER_ERROR");
        const task = (
          await db.select().from(tasks).where(eq(tasks.id, input.id)).limit(1)
        )[0];
    ...
    submitProof: protectedProcedure
      .input(
        z.object({
          ...
          gameUserId: z.string().trim().min(2).max(160),
          screenshotData: z.string().min(24).max(3_000_000),
        })
      )
      .mutation(async ({ ctx, input }) => {
        const { user } = await getActor(ctx);
        ...
        const task = (
          await db
            .select()
            .from(tasks)
            .where(and(eq(tasks.id, input.taskId), eq(tasks.isActive, true)))
            .limit(1)
        )[0];
        if (!task) fail("This task is no longer available.", "NOT_FOUND");
        const pending = (
          await db
            .select({ id: taskProofs.id })
            .from(taskProofs)
            .where(
              and(
                eq(taskProofs.userId, user.id),
                eq(taskProofs.taskId, input.taskId),
                eq(taskProofs.status, "pending")
              )
            )
            .limit(1)
        )[0];
        if (pending)
          fail("You already have a proof pending for this task.", "CONFLICT");
        const screenshotUrl = validateInlineUpload(input.screenshotData, {
          label: "screenshot",
          maxBytes: 2 * 1024 * 1024,
          acceptedType: type => type.startsWith("image/"),
        });
        await db
          .insert(taskProofs)
          .values({
    ...
    myProofs: protectedProcedure.query(async ({ ctx }) => {
      ...
      const proofs = await db
        .select()
        .from(taskProofs)
        .where(eq(taskProofs.userId, user.id))
      ...
      return Promise.all(
        proofs.map(async proof => ({
          ...
          task:
            (
              await db
                .select({ title: tasks.title, rewardCoins: tasks.rewardCoins })
                .from(tasks)
    ...
    startAd: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const nextSlot = Array.from(
          { length: quota },
          (_, index) => index + 1
        ).find(slot => !claimedSlotIds.has(slot));
        if (!nextSlot || input.slot > quota || claimedSlotIds.has(input.slot))
          fail(
            "This rewarded ad has already been watched today or is not available.",
            "FORBIDDEN"
          );
        if (input.slot !== nextSlot)
          fail(
            `Please complete Ad ${nextSlot} before starting another ad.`,
            "FORBIDDEN"
          );
        ...
        return {
          ...
          availableAt: new Date(
            startedAt.getTime() + REWARDED_AD_TIMER_SECONDS * 1000
          ),
          timerSeconds: REWARDED_AD_TIMER_SECONDS,
    ...
    claimAd: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (claimStatus !== "claimable")
          fail(
            claimStatus === "invalidated" ? AD_RETRY_MESSAGE : AD_TIMER_MESSAGE
          );
        const profile = (
          await db
            .select()
        ...
        await db
          ...
          .set({
            earningWalletBalance:
              (profile.earningWalletBalance ?? 0) + session.rewardPkr * 100,
          })
          .where(eq(profiles.userId, user.id));
        await db
          .insert(transactions)
    ...
    create: protectedProcedure
      .input(
        z.object({
          ...
          transactionId: z
            ...
            .min(
              3,
              "You entered wrong deposit number / Transaction ID, please enter correct TID"
            )
            .max(128),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.requestedPackageId) {
          ...
          if (!requestedPackage)
            fail("Requested package was not found.", "NOT_FOUND");
        }
  ...
  withdrawal: router({
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const activePackage = await getActivePackageForUser(user.id);
        const fixedAmounts =
          activePackage?.plan?.tier === "pkr_500"
            ? [600, 1000, 3000, 10000]
        ...
        if (input.currency !== "PKR" || !fixedAmounts.includes(amountPkr))
          fail(
            `Select one of the fixed withdrawal amounts: ${fixedAmounts.join(", ")} PKR.`
          );
  ...
  referral: router({
    get: protectedProcedure.query(async ({ ctx }) => {
        ...
        relationRows.map(async relation => {
          const referred = (
            await db
              ...
              .from(profiles)
              .innerJoin(users, eq(users.id, profiles.userId))
              .where(eq(profiles.userId, relation.referredId))
              .limit(1)
          )[0];
          return {
            ...
            pendingTasks: Math.max(
              0,
              relation.maxTaskReward - relation.totalTasksRewarded
            ),
      ...
      return {
        ...
        totalTaskRewardsPkr:
          relationRows.reduce(
            (total, row) => total + row.totalTaskEarnings,
            0
          ) / 100,
        totalWithdrawCommissionPkr:
          relationRows.reduce(
            (total, row) => total + row.totalWithdrawCommission,
            0
          ) / 100,
        referralEarningsPkr:
          relationRows.reduce(
            (total, row) =>
              total + row.totalTaskEarnings + row.totalWithdrawCommission,
            0
          ) / 100,
  ...
  support: router({
    ...
    chatHistory: protectedProcedure.query(async ({ ctx }) => {
      ...
      return db
        ...
        .where(eq(supportChatMessages.userId, user.id))
    ...
    ask: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const conversation = recent
          ...
          .map(message => ({
            role:
              message.role === "admin" ? ("assistant" as const) : message.role,
            content: message.content,
          }));
        const customRules = await db
          .select()
          .from(supportReplyRules)
          .orderBy(desc(supportReplyRules.updatedAt));
        const matchedRule = customRules.find(rule =>
          input.message.toLowerCase().includes(rule.keyword.toLowerCase())
        );
        ...
        try {
          ...
          const response = await invokeLLM({
            ...
            messages: [
              { role: "system", content: SUPPORT_KNOWLEDGE_BASE },
              ...llmConversation,
            ],
          });
          const generated = contentToText(
            response.choices?.[0]?.message?.content
          );
          if (generated) answer = generated.slice(0, 4000);
        } catch (error) {
          console.warn(
            "[Support] AI response unavailable; using knowledge-base fallback.",
            error
          );
        }
        await db
  ...
  admin: router({
    ...
    financialRequests: protectedProcedure.query(async ({ ctx }) => {
      ...
      const [
        ...
        db
          .select({ referredByUserId: profiles.referredByUserId })
          .from(profiles),
        db
      ...
      const members = new Map(
        memberRows.map(member => [
          ...
          {
            ...member,
            activePackageName: activePackageNames.get(member.userId) ?? null,
          },
        ])
      );
      const packageNames = new Map(
        packageRows.map(plan => [plan.id, plan.name])
      );
      ...
      const remainsVisibleInActiveRequests = (row: {
        status: string;
      ...
      return {
        deposits: depositRows
          .filter(remainsVisibleInActiveRequests)
          .map(row => ({
            ...row,
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.approved) {
          await withDbTransaction(db, async tx => {
            ...
            if (!lockedDeposit)
              fail("Deposit request was not found.", "NOT_FOUND");
            ...
            if (!profile)
              fail("The user's profile was not found.", "NOT_FOUND");
    ...
    reviewWithdrawal: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        await withDbTransaction(db, async tx => {
          ...
          if (!lockedRequest)
            fail("Withdrawal request was not found.", "NOT_FOUND");
          ...
          if (!input.approved) {
            ...
            const relation = (
              await tx
                .select()
                .from(referrals)
                .where(eq(referrals.referredId, request.userId))
                .limit(1)
            )[0];
            if (
              relation?.referrerId &&
              relation.referrerId !== request.userId
            ) {
              ...
              if (referrer) {
                await tx
                  ...
                  .set({
                    earningWalletBalance:
                      (referrer.earningWalletBalance ?? 0) + commissionCoins,
                  })
                  .where(eq(profiles.userId, relation.referrerId));
                await tx
                  .update(referrals)
                  .set({
                    totalWithdrawCommission:
                      relation.totalWithdrawCommission + commissionCoins,
                  })
                  .where(eq(referrals.id, relation.id));
                await tx
                  .insert(transactions)
                  .values({
                    userId: relation.referrerId,
                    type: "referral_withdraw_commission",
                    direction: "credit",
                    amountPkr: request.amountPkr / 10,
                    status: "completed",
                    note: `Referral Withdraw Commission 10%: ${request.amountPkr / 10} PKR from referred withdrawal`,
                    referenceType: "referral_withdraw_commission",
    ...
    deleteWithdrawalHistory: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (!record)
          fail("Withdrawal history record was not found.", "NOT_FOUND");
        if (record.status === "pending")
          fail(
            "Review this withdrawal before deleting its history.",
            "CONFLICT"
          );
    ...
    saveSupportReplyRule: protectedProcedure
      .input(
        z.object({
          id: z.number().int().positive().optional(),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (input.id)
          await db
            .update(supportReplyRules)
            .set({ keyword: input.keyword, message: input.message })
            .where(eq(supportReplyRules.id, input.id));
        else
          await db
            .insert(supportReplyRules)
            .values({ keyword: input.keyword, message: input.message });
        return { success: true };
      }),
    deleteSupportReplyRule: protectedProcedure
      .input(z.object({ id: z.number().int().positive() }))
      .mutation(async ({ ctx, input }) => {
        await getAdmin(ctx);
        const db = await getDb();
    ...
    tasks: protectedProcedure.query(async ({ ctx }) => {
      ...
      return rows.map(row => ({
        ...row,
        profitCoins: Math.max(0, row.hiddenProfit - row.rewardCoins),
      }));
    }),
    saveTask: protectedProcedure
      .input(
        z.object({
          ...
          hiddenProfit: z.number().int().nonnegative(),
          playstoreLink: z.string().url().max(1024),
          isActive: z.boolean(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        ...
        if (!db)
          fail("Database is temporarily unavailable.", "INTERNAL_SERVER_ERROR");
        const mediaType = input.imageData
          ?.match(/^data:([^;]+);/)?.[1]
          ?.startsWith("video/")
          ? "video"
          : "image";
        ...
        const values = {
          ...
          hiddenProfit: input.hiddenProfit,
          playstoreLink: input.playstoreLink,
          isActive: input.isActive,
          ...(imageUrl !== undefined ? { imageUrl, mediaType } : {}),
        };
        if (input.id)
          await db.update(tasks).set(values).where(eq(tasks.id, input.id));
        else
          await db
            .insert(tasks)
            .values({ ...values, imageUrl: imageUrl ?? null, mediaType });
        return { success: true };
      }),
    ...
    taskProofs: protectedProcedure.query(async ({ ctx }) => {
      ...
      return Promise.all(
        proofs.map(async proof => {
          ...
          const member = (
            await db
              .select({ username: profiles.username, email: users.email })
              .from(profiles)
              .innerJoin(users, eq(users.id, profiles.userId))
              .where(eq(profiles.userId, proof.userId))
              .limit(1)
          )[0];
    ...
    reviewTaskProof: protectedProcedure
      .input(
        z.object({
          id: z.number().int().positive(),
          approved: z.boolean(),
          rejectReason: z.string().trim().min(3).max(1000).optional(),
        })
      )
      .mutation(async ({ ctx, input }) => {
        await getAdmin(ctx);
        ...
        if (proof.status !== "pending")
          fail("This task proof has already been reviewed.");
        const task = (
          await db
            .select()
            .from(tasks)
        ...
        await withDbTransaction(db, async tx => {
          if (input.approved) {
            ...
            await tx
              ...
              .set({
                earningWalletBalance:
                  (profile.earningWalletBalance ?? 0) + task.rewardCoins,
              })
              .where(eq(profiles.userId, proof.userId));
            await tx
              .insert(transactions)
              .values({
                userId: proof.userId,
                type: "manual_task_reward",
                direction: "credit",
                amountPkr: Math.floor(task.rewardCoins / 100),
                status: "completed",
                note: `Manual task reward: ${task.rewardCoins} coins`,
                referenceType: "task_proof",
                referenceId: String(proof.id),
              });
            await tx
              .update(taskProofs)
              .set({
              ...
              .where(eq(taskProofs.id, proof.id));
        ...
        await notifyUser(
          db,
          proof.userId,
          input.approved ? "Task proof approved" : "Task proof rejected",
          input.approved
            ? `Your task proof was approved and ${task.rewardCoins} coins were added.`
            : `Your task proof was rejected. Reason: ${input.rejectReason}`
        );
        return { success: true };
      }),
    ...
    supportChats: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const memberRows = await db
          ...
          .from(users)
          .innerJoin(profiles, eq(users.id, profiles.userId));
        const filteredMembers = input?.search
        ...
        const rows = await Promise.all(
          filteredMembers.map(async member => ({
            ...member,
            activePackage:
              (await getActivePackageForUser(member.userId))?.plan.name ?? null,
            messages: await db
              .select()
              .from(supportChatMessages)
              .where(eq(supportChatMessages.userId, member.userId))
              .orderBy(asc(supportChatMessages.createdAt))
              .limit(100),
          }))
        );
